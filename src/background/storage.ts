...
export async function seedSkillsIfEmpty(): Promise<void> {
  ...
  const examples: Skill[] = [
    ...
    },
    {
      ...
      name: 'search-mail',
      description:
        'Search your Outlook mail via the microsoft365_search tool (REST over the signed-in session) — filter by sender, time, and keywords; falls back to the Outlook web UI if the endpoint errors.',
      body: [
