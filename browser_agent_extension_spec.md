...
- On-device RAG (post-MVP): named repositories in OPFS holding captured pages' chunk text + int8-quantized embedding vectors. add_to_repo captures the active tab or the conversation's tab group; search_repo embeds a query and retrieves the nearest passages for the agent to answer from with citations; list_repos + a Settings section manage them. Embeddings via the configured endpoint's /embeddings route (turbovec-inspired scalar quantization; int8 now, 4-bit/2-bit as a future optimization). Needs unlimitedStorage; runs in the offscreen document. OCR-fallback ingestion (vision transcription) and ingest buttons are a follow-on. PDFs are ingested via pdf.js before the DOM/OCR ladder. De-duplication: re-adding a page (matched by URL ignoring query/hash) prompts to replace the existing copy (one combined prompt for group adds; decline keeps the original); individual documents can be deleted from the Settings → Repositories section.
- Per-conversation tab groups (post-MVP): tabs the agent opens (search_web, a new open_url tool) are collected into a Chrome tab group with a single-word name, one group per conversation; a read_tab_group tool lets the user/agent reference a group by name to read all its pages. Requires the tabGroups permission; previous conversations' groups are left open.
