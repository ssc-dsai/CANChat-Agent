...
import { SCENES, type SceneDef } from './narration.ts';
...
async function recordScene(id: string, mockBase: string): Promise<Captured> {
  ...
  try {
    ...
    await spec.run({ context, serviceWorker: sw, extensionId, mockBase, page, marks, sceneStart: Date.now() });
...
function writeScript(rows: Array<{ id: string; start: number; end: number }>): void {
  const lines: string[] = [
    ...
    'the finished cut. Narration lives in `scripts/demo/narration.ts` as per-scene',
    'beats anchored to on-screen checkpoints; edit there and re-record.',
    '',
  ...
  for (const r of rows) {
    ...
    const text = def.beats.map((b) => b.text).join(' ');
